...
yyaxis left
plot(ODrange, LasB/1000, LineWidth = 2, Color = LasBColor);
ax = gca;
ax.FontName = FontName;
ax.YColor = [0 0 0];
xlabel({'Population Density (~ OD600)'}, FontName=FontName, FontSize=9);
xtickformat('%.1f')
ylabel({'Predicted Expression', '(Per-Capita 10^{3} RLU/OD)'}, FontName=FontName, FontSize=9);
ylim(Yrange/1000);

yyaxis right
scatter(AggregateData.od, AggregateData.mean_expression, 30, RhlIColor, LineWidth=1);
ylabel({'Observed Response', '(Per-Capita Pixel Intensity)'}, FontName=FontName, FontSize=9);
ax = gca;
ax.FontName = FontName;
ax.YColor = [0 0 0];
ylim(Xrange(2,:));
title("\it lasB\rm Expression", FontName=FontName, FontWeight="normal", FontSize=11);
legend({"Model", "Observations"}, FontName=FontName, FontSize=8, Location='northwest');
...
LasBValues = NMtable(NMtable.Architecture == "reciprocal", :).lasB / 1000;
...
MakeHeatmap( ...
    dataMatrix=getMatrix('lasB', "reciprocal", false) / 1000, ...
    dataRange=LasBRange, ...
    xRange=Mrange, ...
    yRange=Nrange, ...
    thresholds = [0.05, 0.5], ...
...
MakeHeatmap( ...
    dataMatrix=getMatrix('lasB', "hierarchical", false) / 1000, ...
    dataRange=LasBRange, ...
    xRange=Mrange, ...
    yRange=Nrange, ...
    thresholds = [0.05, 0.5], ...
...
MakeHeatmap( ...
    dataMatrix=getMatrix('lasB', "independent", false) / 1000, ...
    dataRange=LasBRange, ...
    xRange=Mrange, ...
    yRange=Nrange, ...
    thresholds = [0.05, 0.5], ...
    ...
    yLabel="", ...
    legendLabel="\it lasB\rm\rm (10^{3} RLU/OD)" ...
);
...
MakeHeatmap( ...
    dataMatrix=getMatrix('lasB', "hierarchical", true) / 1000, ...
    dataRange=LasBRange, ...
    xRange=Mrange, ...
    yRange=Nrange, ...
    thresholds = [0.05, 0.5], ...
...
MakeHeatmap( ...
    dataMatrix=getMatrix('lasB', "independent", true) / 1000, ...
    dataRange=LasBRange, ...
    xRange=Mrange, ...
    yRange=Nrange, ...
    thresholds = [0.05, 0.5], ...
...
exportgraphics(LasB_Heatmaps, "../Prefigures/lasb_heatmaps.pdf", 'ContentType', 'vector');
...
% MakeHeatmap creates a heatmap from a matrix of data values
    ...
    Heatmap = gca;
    set(Heatmap,'XTickLabel', xTicks, 'fontsize', 10 * fontScale, 'FontName', FontName);
    xlabel(xLabel, FontName=FontName, FontSize=10*fontScale);
    if yLabel ~= ""
        ylabel(yLabel, FontName=FontName, FontSize=10*fontScale);
    end
    if plotTitle ~= ""
        title(plotTitle, FontName=FontName, FontWeight="normal", FontSize=11*fontScale);
    end
    if plotSubtitle ~= ""
        text(-0.1, 1.1, plotSubtitle, FontName=FontName, FontSize=11*fontScale, FontWeight="normal", Units="normalized");
    end
    ...
    for tIdx = 1 : length(tLevels)
        ...
        text(median(tX(tIdx, :)), median(tY(tIdx, :)), label, ...
            Color=tColors{tIdx}, FontName=FontName, FontSize=10*fontScale, ...
    ...
    if legendLabel ~= ""
        ...
        Colorbar.Label.FontSize = 10*fontScale;
